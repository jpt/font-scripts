...
# This currently makes two designspaces. Todo: update to designspaceLib 5 way of doing it
...
to_decompose = []  # if you want, make an array of glyphnames (not unicodes)
to_remove_overlap = []  # same


# Careful editing beyond here!

# Skateboard layers Todo
# Set to true if you prefer your intermediate masters on layers (Skateboard "support" layers) rather than in separate UFOs
# skateboard_layers = False
# # Production names todo
# Set to true for production unicode glyph names — need to figure out how to not mess up feature code this way. probably switch the font's glyph names first - but if features are not set to autoupdate, then what? maybe dont worry about it and just let fontmake rename?
...
def addBuildScript(font, dest):
	...
	static_script = f"""python3 -m fontmake -i -m {static_font_name}.designspace -o ttf --output-dir build/ttf
	python3 -m fontmake -i -m {static_font_name}.designspace -o otf --output-dir build/otf"""
	if to_build["static"] and to_build["variable"]:
		build_script = "#!/bin/bash" + "\n" + vf_script + "\n" + static_script + "\n"
	elif to_build["static"]:
		build_script = "#!/bin/bash" + "\n" + static_script + "\n"
	else:
		build_script = "#!/bin/bash" + "\n" + vf_script + "\n"
...
def generateMastersAtBraces(font, temp_project_folder, format):
	...
	for i, special_layer_axis in enumerate(special_layer_axes):
		...
		ufo = buildUfoFromMaster(brace_font.masters[0], brace_font.glyphs)
		ufo.save(ufo_file_path)


def getIndexByMaster(master):
...
def buildUfoFromMaster(master, glyphs):
	__doc__ = """Provided a GSMFontMaster and a list of glyph names to be used in that master, return a fontParts UFO"""
	font = master.font
	master_index = getIndexByMaster(master)

	print("Building master: %s - %s" % (master.font.familyName, master.name))
...
def exportUFOMasters(font, dest, format):
	...
	for master in font.masters:
		...
		ufo = buildUfoFromMaster(master, font.glyphs)
		ufo = addGroups(font, ufo)
		ufo = addKerning(font, ufo)
		ufo = addFeatureInclude(ufo, font)
		ufo.save(ufo_file_path)


def getFeatureDict(font):
	...
	# add size feature for use in build script
...
def writeFeatureFiles(font, dest):
	...
	with tempfile.TemporaryDirectory() as tmp_dir:
		...
		if to_build["static"] or (to_build["variable"] and not hasVariableFamilyName(font)):
			...
			static_designspace_path = "%s/%s.designspace" % (
				temp_project_folder, getFamilyName(font, "static"))
		...
		if to_build["variable"] and hasVariableFamilyName(font):
			...
			variable_designspace_path = "%s/%s.designspace" % (
				temp_project_folder, getFamilyName(font, "variable").replace(" ", ""))
		...
		writeFeatureFiles(font, temp_project_folder)
