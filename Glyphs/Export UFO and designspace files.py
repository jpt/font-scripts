...
# Set to False if you want brace layers to be full masters and not just affected glyphs
delete_unnecessary_glyphs = True
use_production_names = False  # Set to true for production unicode glyph names
# Not sure what happens if this is set to False. Good luck! :D
decompose_smart_stuff = True
# Needed for ScaleFast and potentially other Robofont plugins
...
def getFamilyName(font):
    ...
    if is_vf:
        if getVariableFontFamily(font) == font.familyName:
...
def getNameWithAxis(font, axes):
    ...
    if is_vf:
        if getVariableFontFamily(font) == font.familyName:
            ...
            font_name = "%s %s -" % (font.familyName,
                                     getVariableFontFamily(font))
    else:
